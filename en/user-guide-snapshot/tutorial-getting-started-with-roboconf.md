...
In Karaf's command line interface, execute the following commands. We here assume you use Roboconf %v_SNAP%.

```tcl
bundle:install mvn:net.roboconf/roboconf-plugin-api/%v_SNAP%
bundle:install mvn:net.roboconf/roboconf-agent/%v_SNAP%
bundle:install mvn:net.roboconf/roboconf-target-in-memory/%v_SNAP%
```
...
Download it [here](https://bintray.com/artifact/download/roboconf/roboconf-tutorial-samples/lamp-webapp-bash-%v_SNAP%.0-1.0.zip). It is a ZIP archive with a specific structure 
